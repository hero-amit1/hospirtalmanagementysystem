...
if (isset($_POST['patsub1'])) {
  ...
  $cpassword = $_POST['cpassword'] ?? '';
    ...
    // Check for duplicate email/contact via prepared statement
    $existing = null;
    $check_stmt = mysqli_prepare($con, "SELECT pid,fname,lname,gender,email,contact,password FROM patreg WHERE email=? OR contact=? LIMIT 1");
    if ($check_stmt) {
      mysqli_stmt_bind_param($check_stmt, "ss", $email, $contact);
    ...
    if ($existing) {
      $sameUser = ($existing['email'] === $email && $existing['contact'] === $contact);
      $looksTruncatedHash = (strpos($existing['password'], '$2y$') === 0 && strlen($existing['password']) < 60);

      // Recovery path for accounts created when DB hash columns were too short.
      if ($sameUser && $looksTruncatedHash && empty($errors)) {
        $newHash = password_hash($password, PASSWORD_DEFAULT);
        $repair_stmt = mysqli_prepare($con, "UPDATE patreg SET fname=?, lname=?, gender=?, password=?, cpassword=? WHERE pid=?");
        if ($repair_stmt) {
          mysqli_stmt_bind_param($repair_stmt, "sssssi", $fname, $lname, $gender, $newHash, $newHash, $existing['pid']);
          $repaired = mysqli_stmt_execute($repair_stmt);
          mysqli_stmt_close($repair_stmt);

          if ($repaired) {
            session_regenerate_id(true);
            $_SESSION['pid'] = (int)$existing['pid'];
          ...
          $errors[] = "Account recovery failed. Please try again.";
        } else {
          $errors[] = "Account recovery failed. Please try again.";
        }
      } else {
    ...
    if (count($errors) > 0) {
        // Handle errors (redirect or display)
        $_SESSION['reg_errors'] = $errors;
        header("Location:register-error.php"); // Create this page to show errors
        exit();
    } else {
        // Hash the password
        ...
        $insert_stmt = mysqli_prepare($con, "INSERT INTO patreg (fname, lname, gender, email, contact, password, cpassword) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $result = false;
        $newPid = 0;
        if ($insert_stmt) {
          mysqli_stmt_bind_param($insert_stmt, "sssssss", $fname, $lname, $gender, $email, $contact, $hashed_password, $hashed_password);
