...
if(isset($_POST['patsub'])){
  ...
  if($stmt){
    ...
    if(mysqli_stmt_num_rows($stmt) === 1){
      mysqli_stmt_bind_result($stmt, $pid, $fname, $lname, $gender, $contact, $dbEmail, $storedPassword);
      mysqli_stmt_fetch($stmt);
      $isValidPassword = password_verify($password, $storedPassword) || hash_equals($storedPassword, $password);
      if($isValidPassword){
        $passwordInfo = password_get_info($storedPassword);
        $isLegacyPassword = !is_array($passwordInfo) || (($passwordInfo['algoName'] ?? 'unknown') === 'unknown');
        if($isLegacyPassword){
